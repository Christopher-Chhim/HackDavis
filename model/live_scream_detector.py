...
def predict_scream(yamnet_model, classifier_model, threshold=0.5, waveform_data=None):
    ...
    if not isinstance(waveform_data, np.ndarray) or waveform_data.ndim != 1:
         print(f"Debug: Invalid waveform_data type/dims: {type(waveform_data)}, ndim={getattr(waveform_data, 'ndim', 'N/A')}")
    ...
    try:
        ...
        print("Warning: Output key 'dense_2' not found. Attempting to access first output.")
        try:
    ...
    try:
        while True:
            ...
            try:
                ...
                recording_data = sd.rec(int(CHUNK_DURATION * SAMPLE_RATE),
                                        samplerate=SAMPLE_RATE,
                                        channels=CHANNELS,
                                        dtype='float32',
                                        blocking=True) # Use blocking=True for simplicity
                ...
                # Perform inference
                label, probability = predict_scream(yamnet_model=yamnet_model,
                                                    classifier_model=classifier_model,
                                                    waveform_data=recording_flat)

                # Display result
                print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Prediction: {label} (Probability: {probability:.4f})")
